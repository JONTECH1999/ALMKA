...
// Address cache for coordinates - fallback when API fails
const ADDRESS_CACHE = {
    '14.772593,121.078935': 'Sitio Pag-asa, Caloocan City, Metro Manila, Philippines',
};

// Function to get address from cache or API
const getAddressForCoordinates = async (latitude, longitude) => {
    const cacheKey = `${latitude.toFixed(6)},${longitude.toFixed(6)}`;
    
    // Check exact cache first
    if (ADDRESS_CACHE[cacheKey]) {
        return ADDRESS_CACHE[cacheKey];
    }
    
    // Check nearby cache (within ~0.001 degrees ≈ 100 meters)
    for (const [key, address] of Object.entries(ADDRESS_CACHE)) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (addr.industrial || addr.artisan) addressParts.push(addr.industrial || addr.artisan);
        ...
        console.warn('Address lookup failed, using fallback:', err);
    }
...
export default function LiveMonitoring() {
    ...
    const startLocationWatch = useCallback(() => {
        ...
        const watchId = navigator.geolocation.watchPosition(
            async (position) => {
                ...
                const address = await getAddressForCoordinates(position.coords.latitude, position.coords.longitude);
    ...
    // Start location watch on mount and save every 3 minutes
    useEffect(() => {
        startLocationWatch();
    ...
    // Auto-record a short clip every 3 minutes when stream starts
    useEffect(() => {
        if (!recordingStarted) return;

        console.log('Starting auto video recording every 3 minutes...');
        const interval = setInterval(() => {
    ...
    useEffect(() => {
        if (!recordingStarted) {
            console.log('Auto-starting video recording...');
    ...
    const checkDeviceDiagnostics = useCallback(async () => {
        ...
        const url = `http://${espIp.trim()}:81/status`;
        try {
            const response = await fetch(url, { timeout: 3000 });
            if (response.ok) {
                setDeviceInfo({
                    status: 'connected',
                    message: `✓ Device connected at ${espIp}`,
                });
            } else {
                setDeviceInfo({
                    status: 'error',
                    message: `Device returned error (${response.status})`,
                });
            }
        } catch (err) {
            setDeviceInfo({
                        ...
                        <section className="rounded-3xl bg-white p-8 shadow-xl border border-blue-100">
                            ...
                            <div className="mt-6 rounded-2xl bg-blue-50 border border-blue-200 p-4">
                                ...
                                <p className="mt-2 text-sm text-blue-900 leading-relaxed">
                                    ALMKA Device - WiFi connected! IP Address: 10.88.109.105<br />
